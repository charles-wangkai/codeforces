...
public class Main {
  ...
  static BigInteger solve(int n, int k) {
      ...
      for (int c = 0; c < n; ++c) {
        ...
        for (int i = 0; i <= k; ++i) {
          for (int state = 0; state < 1 << (n + 1); ++state) {
            ...
            if (i != k
                && (c == 0 || ((state & 1) == 0 && (state & (1 << n)) == 0))
                && (c == n - 1 || (state & (1 << (n - 2))) == 0)
                && (state & (1 << (n - 1))) == 0) {
              nextWayNums[i + 1][nextState + 1] =
                  nextWayNums[i + 1][nextState + 1].add(wayNums[i][state]);
            }
          }
        }
    ...
    return Arrays.stream(wayNums[k]).reduce(BigInteger::add).get();
  }
}
