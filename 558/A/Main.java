...
public class Main {
  ...
  static int solve(int[] x, int[] a) {
    ...
    return Math.max(
        computeSum(
                positionToCount, positives, commonSize + (commonSize == positives.size() ? 0 : 1))
            + computeSum(positionToCount, negatives, commonSize),
        computeSum(positionToCount, positives, commonSize)
            + computeSum(
                positionToCount, negatives, commonSize + (commonSize == negatives.size() ? 0 : 1)));
  }

  static int computeSum(Map<Integer, Integer> positionToCount, List<Integer> positions, int limit) {
    return positions.stream().limit(Math.max(0, limit)).mapToInt(positionToCount::get).sum();
  }
}
