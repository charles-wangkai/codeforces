...
public class Main {
	...
	static String solve(int n, int[] u, int[] v) {
		...
		for (int i = 0; i < n; i++) {
			if (!indexToGroup.containsKey(i)) {
				if (!search(adjacentLists, indexToGroup, i, 0)) {
					return "-1";
				}
			}
		...
		return IntStream.range(0, vertices.length)
				.mapToObj(i -> String.format("%d\n%s", vertices[i].size(),
						vertices[i].stream().map(x -> String.valueOf(x + 1)).collect(Collectors.joining(" "))))
	...
	static boolean search(List<Integer>[] adjacentLists, Map<Integer, Integer> indexToGroup, int index, int group) {
